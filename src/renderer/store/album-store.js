...
export function match(album, filter) {
  return _.every(filter, (value, key) => {
    ...
    return _.some(album.matchData[key], (targetValue) =>
      new RegExp(value, "i").test(targetValue),
    )
  })
}
          ...
          autorun(
            () => {
              ...
              if (
                ...
                self.setMatches(
                  activeDevice.activeSection.albums.filter((album) =>
                    match(album, self.filterSet),
                  ),
                )
